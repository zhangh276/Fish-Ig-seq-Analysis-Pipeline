...
# Step 02: MiXCR alignment and CDR3-based clonotype assembly.
...
OUT_DIR="${3:-results/02_cdr3_clones}"
...
do
  echo "Running MiXCR CDR3 clonotype analysis for sample: ${sample_id}"
  ...
  "${MIXCR_BIN}" analyze generic-amplicon \
    ...
    "${R1_CORRECTED}" \
    "${R2_RC}" \
    "${OUT_DIR}/${sample_id}_CDR3"

  echo "CDR3 clonotype analysis finished for sample: ${sample_id}"
done

echo "Step 02 finished: MiXCR CDR3 clonotype analysis completed."
